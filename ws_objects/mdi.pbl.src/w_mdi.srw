...
type st_1 from statictext within w_mdi
end type
...
il_count ++
This.Title = This.title + string(il_count)
end event
...
mdi_1.resize(this.workspaceWidth() - st_1.width - 8 ,this.workspaceHeight() - rbb_1.height - 20 -60 )
...
If rbb_1.GetItem(index, lr_LargeButton) = 1 Then
	choose case lr_largeButton.text
		case "Order"
			Opensheet (lw_child2, Parent, 1, Layered! )
		case "Task"
			Opensheet (lw_child3, Parent, 1, Layered! )
		Case "Report"
			Opensheet (lw_child4, Parent, 1, Layered! )
		Case else
